@extends('layouts.app')
@section('title', 'Registrar asignaturas')
	...
	<div class="card">
		<div class="card-header">
			<h1 class="typography-headline">
				<i class="material-icons mr-1">book</i> Registrar Asignaturas
			</h1>
		</div>
		<div class="card-body">
			...
			<form method="post" action="{{ route('asignaturas.store') }}" autocomplete="off">
				...
				<blockquote class="blockquote my-3">
				  <p class="mb-0 typography-subheading">Información del Asignaturas</p>
				  <hr class="w-100">
				</blockquote>
					      	...
					      	@foreach($grados as $grado)
								 	<option value="{{ $grado->id }}"
					      				@if (old('grado_id') === $grado->id){{ 'selected' }}@endif>
					      					{{ $grado->nomb_grad }} &middot; Jornada {{ $grado->getJornada() }}
					      			</option>
					      	@endforeach
					    </select>
				...
				<div class="form-row">
				  	<div class="form-group col-md-12">
			    		<label>Logros 1er Periodo</label>
			    		<textarea name="log1_asig" rows="3" class="form-control {{ $errors->has('log1_asig') ? 'is-invalid' : '' }}" autofocus>
			    			{{ old('log1_asig') }}
			    		</textarea>
				...
				<div class="form-row">
				  	<div class="form-group col-md-12">
			    		<label>Logros 2do Periodo</label>
			    		<textarea name="log2_asig" rows="3" class="form-control {{ $errors->has('log2_asig') ? 'is-invalid' : '' }}" autofocus>
			    			{{ old('log2_asig') }}
			    		</textarea>
				...
				<div class="form-row">
				  	<div class="form-group col-md-12">
			    		<label>Logros 3er Periodo</label>
			    		<textarea name="log3_asig" rows="3" class="form-control {{ $errors->has('log3_asig') ? 'is-invalid' : '' }}" autofocus>
			    			{{ old('log3_asig') }}
			    		</textarea>
				...
				<div class="form-row">
				  	<div class="form-group col-md-12">
			    		<label>Logros 4to Periodo</label>
			    		<textarea name="log4_asig" rows="3" class="form-control {{ $errors->has('log4_asig') ? 'is-invalid' : '' }}" autofocus>
			    			{{ old('log4_asig') }}
			    		</textarea>
