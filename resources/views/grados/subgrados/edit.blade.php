...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb bg-white shadow-1">
    ...
    <li class="breadcrumb-item"><a href="{{ route('grados.subgrados.edit', [$grado->id, $subgrado->id]) }}">
    	Subgrados <i class="material-icons">chevron_right</i> Editar
    </a></li>
...
<div class="card">
	<div class="card-header bg-light-2">
  		<h1 class="typography-headline">
  			<i class="material-icons mr-1">school</i> Editar subgrado
  		</h1>
  	</div>
	<div class="card-body">
		...
		<form  method="post" action="{{ route('grados.subgrados.update', [$grado->id, $subgrado->id]) }}" autocomplete="off">
			{{ csrf_field() }}
			{{ method_field('PUT') }}
			...
			<div class="form-row">
				<div class="form-group col-md-3">
			    	<label>Abreviación (letra o número)</label>
			   		<input type="text" name="abre_subg" class="form-control {{ $errors->has('abre_subg') ? 'is-invalid' : '' }}" value="{{ old('abre_subg', $subgrado->abre_subg) }}" required autofocus>
		  		...
		  		<div class="form-group col-md-3">
			    	<label>Máximo número de estudiantes</label>
			   		 <input type="number" name="cant_estu" class="form-control {{ $errors->has('cant_estu') ? 'is-invalid' : '' }}" value="{{ old('cant_estu', $subgrado->cant_estu) }}" required autofocus>
				      	...
				      	@foreach($docentes->chunk(15) as $chunk)
			                @foreach($chunk as $docente)
			                  	<option value="{{ $docente->id }}"
			                    	@if (old('docente_id', $subgrado->getDirectorId()) === $docente->id){{ 'selected' }}@endif>
