        ...
        <form method="post" action="{{ route('roles.update', $role->id) }}" autocomplete="off">
            ...
            <div class="form-row">
                <div class="form-group col-md-6">
                    <label>Nombre</label>
                     <input type="text" name="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" value="{{ old('name', $role->name) }}" required autofocus>
                    @if ($errors->has('name'))
                ...
                <div class="form-group col-md-6">
                    ...
                    <input type="text" name="slug" class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" value="{{ old('slug', $role->slug) }}" required autofocus
                        @unless($role->noEsRolPrincipal()){{ 'readonly' }}@endunless>
                    @if ($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <div class="mt-3">
                        @foreach (SpecialRole::asociativos() as $special)
                            <div class="custom-control custom-radio custom-control-inline">
                                <input type="radio" id="special{{ $loop->index }}" name="special"
                                    class="custom-control-input {{ $errors->has('special') ? 'is-invalid' : '' }}"
                                    value="{{ $special['id'] }}"
                                    {{ old('special', $role->special) === $special['id'] ? 'checked' : '' }}
                                    @if ($role->deshabilitarSpecial($special['id'])){{ 'disabled' }}@endif>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <textarea name="description" rows="3" class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" autofocus>{{ old('description', $role->description) }}</textarea>
                                ...
                                <li class="list-group-item list-group-item-action">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox"
                                            ...
                                            value="{{ $permission->id }}"
                                            {{ old('permissions.' . $permission->id, $role->getPermisssionId($permission->id)) === $permission->id ? 'checked' : '' }}
                                            @if ($role->isAllAccess()){{ 'disabled' }}@endif>
