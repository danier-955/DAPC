...
<div class="card">
	<div class="card-header">
		<h1 class="typography-headline">
			<i class="material-icons mr-1">group</i> Editar practicante
		</h1>
	</div>
	<div class="card-body">
		...
		<form method="post" action="{{ route('practicantes.update', $practicante->id) }}" autocomplete="off">
				...
				<div class="form-group col-md-3">
	    			...
	    			<select name="tipo_docu" class="selectpicker dropdown-dense show-tick selectbox form-control {{ $errors->has('tipo_docu') ? 'is-invalid' : '' }}" required autofocus>
	      				@foreach(Documento::tipos() as $tipo)
	      					<option value="{{ $tipo }}"
	      						@if (old('tipo_doce', $practicante->tipo_docu) === $tipo){{ 'selected' }}@endif>
	      						{{ $tipo }}
	      					</option>
	      				@endforeach
	    			</select>
			...
			<div class="form-row">
			  	<div class="form-group col-md-3">
			    	<label>Segundo apellido</label>
			   		 <input type="text" name="sape_prac" class="form-control {{ $errors->has('sape_prac') ? 'is-invalid' : '' }}" value="{{ old('sape_prac', $practicante->sape_prac) }}"  autofocus>
				    ...
				    <select name="sexo_prac" class="selectpicker dropdown-dense show-tick selectbox form-control {{ $errors->has('sexo_prac') ? 'is-invalid' : '' }}" required autofocus>
				    	@foreach(Sexo::asociativos() as $sexo)
				      		<option value="{{ $sexo['id'] }}"
				      			@if (old('sexo_prac', $practicante->sexo_prac) === $sexo['id']){{ 'selected' }}@endif>
				      			{{ $sexo['texto'] }}
				      		</option>
				      	@endforeach
				    </select>
			  	...
			  	<div class="form-group col-md-3">
			    	<label>Barrio de residencia</label>
			   		<input type="text" name="barr_prac" class="form-control {{ $errors->has('barr_prac') ? 'is-invalid' : '' }}" value="{{ old('barr_prac', $practicante->barr_prac) }}" autofocus>
			...
			<div class="form-row">
			  	<div class="form-group col-md-3">
			    	<label>Correo electrónico</label>
			   		<input type="email" name="corr_prac" class="form-control {{ $errors->has('corr_prac') ? 'is-invalid' : '' }}" value="{{ old('corr_prac', $practicante->corr_prac) }}" required autofocus>
			 	...
			 	<div class="form-group col-md-3">
			    	<label>Teléfono</label>
			   		<input type="number" name="tele_prac" class="form-control {{ $errors->has('tele_prac') ? 'is-invalid' : '' }}" value="{{ old('tele_prac', $practicante->tele_prac) }}" autofocus>
				...
				<div class="form-group col-md-3">
			    	<label>Nombre del padre</label>
			   		<input type="text" name="padr_prac" class="form-control {{ $errors->has('padr_prac') ? 'is-invalid' : '' }}" value="{{ old('padr_prac', $practicante->padr_prac) }}" autofocus>
			  	...
			  	<div class="form-group col-md-3">
			    	<label>Nombre de la madre</label>
			   		<input type="text" name="madr_prac" class="form-control {{ $errors->has('madr_prac') ? 'is-invalid' : '' }}" value="{{ old('madr_prac', $practicante->madr_prac) }}" autofocus>
				...
				<div class="form-group col-md-4">
				    ...
				    <select name="sub_grado_id" class="selectpicker dropdown-dense show-tick selectbox form-control {{ $errors->has('sub_grado_id') ? 'is-invalid' : '' }}" data-live-search="true" data-live-search-placeholder="Búscar ..." required autofocus>
	    				@empty(old('sub_grado_id'))
	    				    <option value="{{ $practicante->sub_grado_id }}" selected>
	    				    	{{ $practicante->getSubGradoNombre() }}
	    				    </option>
	    				@endempty
				      	@foreach($grados as $grado)
							@foreach($grado->subGrados as $subGrado)
							 	<option value="{{ $subGrado->id }}"
				      				@if (old('sub_grado_id', $practicante->getSubGradoId()) === $subGrado->id){{ 'selected' }}@endif>
			...
			<div class="form-row">
			  	<div class="form-group col-md-12">
		    		...
		    		<textarea name="obse_prac" rows="3" class="form-control {{ $errors->has('obse_prac') ? 'is-invalid' : '' }}" autofocus>{{ old('obse_prac', $practicante->obse_prac) }}</textarea>
