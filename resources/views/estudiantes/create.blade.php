			...
			<form method="post" action="{{ route('estudiantes.store') }}" autocomplete="off" enctype="multipart/form-data">
				...
				<div class="form-row">
					<div class="form-group col-md-3">
				    	<label>Barrio de residencia</label>
				   		<input type="text" name="barr_estu" class="form-control {{ $errors->has('barr_estu') ? 'is-invalid' : '' }}" value="{{ old('barr_estu') }}" autofocus>
				...
				<div class="form-row">
			        <div class="form-group col-md-12">
			            ...
			            <textarea name="obse_estu" rows="3" class="form-control" autofocus>{{ old('obse_estu') }}</textarea>
				...
				<div class="form-row">
					<div class="form-group col-md-9 {{ $errors->has('acudiente_id') ? 'has-error' : '' }}">
                       <label class="col-form-label">
                       		Acudientes registrados
                       		<small class="text-muted">Búscar acudiente por documento, nombres o apellidos</small>
                       	</label>
                       	<select id="acudiente_id" name="acudiente_id" class="form-control {{ $errors->has('acudiente_id') ? 'is-invalid' : '' }}">
                       		@empty(old('acudiente_id'))
                       			<option value="">--- Seleccione ---</option>
                       		@else
                            	<option value="{{ old('acudiente_id') }}">{{ acudienteSelect2(old('acudiente_id')) }}</option>
                            @endempty
                        </select>
				...
				<div class="form-row">
					<div class="form-group col-md-3">
			    		<label>Tipo de documento</label>
		    			<select  id="tipo_docu" name="tipo_docu" class="selectpicker dropdown-dense show-tick selectbox form-control{{ $errors->has('tipo_docu') ? 'is-invalid' : '' }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'disabled' }}>
		    				@empty(old('tipo_docu'))
			                    <option value="">··· Seleccione ···</option>
			                @endempty
					...
					<div class="form-group col-md-3">
				    	<label>No. Identificación</label>
				   		 <input id="docu_acud" type="number" name="docu_acud" class="form-control {{ $errors->has('docu_acud') ? 'is-invalid' : '' }}" value="{{ old('docu_acud') }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'readonly' }}>
					...
					<div class="form-group col-md-3">
				    	<label>Nombres</label>
				   		<input  id="nomb_acud" type="text" name="nomb_acud" class="form-control {{ $errors->has('nomb_acud') ? 'is-invalid' : '' }}" value="{{ old('nomb_acud') }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'readonly' }}>
				  	...
				  	<div class="form-group col-md-3">
				    	<label>Primer apellido</label>
				   		<input id="pape_acud" type="text" name="pape_acud" class="form-control {{ $errors->has('pape_acud') ? 'is-invalid' : '' }}" value="{{ old('pape_acud') }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'readonly' }}>
					...
					<div class="form-group col-md-3">
				    	<label>Segundo apellido</label>
				   		<input id="sape_acud" type="text" name="sape_acud" class="form-control {{ $errors->has('sape_acud') ? 'is-invalid' : '' }}" value="{{ old('sape_acud') }}" autofocus
				   			{{ empty(old('acudiente_id')) ? '' : 'readonly' }}>
				  	...
				  	<div class="form-group col-md-3">
					    <label>Sexo</label>
					    <select id="sexo_acud" name="sexo_acud" class="selectpicker dropdown-dense show-tick selectbox form-control {{ $errors->has('sexo_acud') ? 'is-invalid' : '' }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'disabled' }}>
					    	<option value="">··· Seleccione ···</option>
					    	@foreach(Sexo::asociativos() as $sexo)
					      		<option value="{{ $sexo['id'] }}"
			 		...
			 		<div class="form-group col-md-3">
				    	<label>Dirección de residencia</label>
				   		<input id="dire_acud" type="text" name="dire_acud" class="form-control {{ $errors->has('dire_acud') ? 'is-invalid' : '' }}" value="{{ old('dire_acud') }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'readonly' }}>
				...
				<div class="form-row">
					<div class="form-group col-md-3">
				    	<label>Correo electrónico</label>
				   		<input id="corr_acud" type="email" name="corr_acud" class="form-control {{ $errors->has('corr_acud') ? 'is-invalid' : '' }}" value="{{ old('corr_acud') }}" autofocus
				   			{{ empty(old('acudiente_id')) ? 'required' : 'readonly' }}>
