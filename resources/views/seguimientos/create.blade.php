...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb bg-white shadow-1">
    ...
    <li class="breadcrumb-item"><a href="{{ route('seguimientos.create') }}">Registrar</a></li>
...
<div class="card">
	<div class="card-header bg-light-2">
		<h1 class="typography-headline">
			<i class="material-icons mr-1">file_copy</i> Registrar seguimiento
		</h1>
	</div>
	<div class="card-body">

		<form method="post" action="{{ route('seguimientos.store') }}" autocomplete="off">
			{{ csrf_field() }}
	      				...
	      				@foreach($practicantes->chunk(15) as $chunk)
		      				@foreach($chunk as $practicante)
		      					<option value="{{ $practicante->id }}"
		      						@if (old('practicante_id') === $practicante->id){{ 'selected' }}@endif>
			...
			<div class="form-row">
				<div class="form-group col-md-3">
			    	<label>Fecha</label>
			    	<input type="text" name="fech_segu" class="datepicker form-control {{ $errors->has('fech_segu') ? 'is-invalid' : '' }}" value="{{ old('fech_segu') }}" required autofocus>
				...
				<div class="form-group col-md-3">
			    	...
			    	<input type="text" name="hora_lleg" class="timepicker form-control {{ $errors->has('hora_lleg') ? 'is-invalid' : '' }}" value="{{ old('hora_lleg') }}" required autofocus>
				...
				<div class="form-group col-md-3">
			    	...
			    	<input type="text" name="hora_sali" class="timepicker form-control {{ $errors->has('hora_sali') ? 'is-invalid' : '' }}" value="{{ old('hora_sali') }}" required autofocus>
			  	...
			  	<div class="form-group col-md-3">
			    	<label>Horas cumplidas</label>
			   		<input type="number" name="hora_cump" class="form-control {{ $errors->has('hora_cump') ? 'is-invalid' : '' }}" value="{{ old('hora_cump') }}" required autofocus>
			...
			<div class="form-row">
			  	<div class="form-group col-md-12">
		    		...
		    		<textarea name="acti_real" rows="3" class="form-control {{ $errors->has('acti_real') ? 'is-invalid' : '' }}" required autofocus>{{ old('acti_real') }}</textarea>
			...
			<div class="form-row">
			  	<div class="form-group col-md-12">
		    		...
		    		<textarea name="obse_segu" rows="3" class="form-control {{ $errors->has('obse_segu') ? 'is-invalid' : '' }}" autofocus>{{ old('obse_segu') }}</textarea>
